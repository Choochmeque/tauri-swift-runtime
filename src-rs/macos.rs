...
swift!(pub fn swift_run_plugin_command(
  id: i32,
  name: &SRString,
  method: &SRString,
  data: &SRString,
...
swift!(pub fn swift_register_plugin(
  name: &SRString,
  plugin: *const c_void,
  config: &SRString,
  webview: *const c_void
));
swift!(pub fn swift_on_webview_created(webview: *const c_void, controller: *const c_void));
